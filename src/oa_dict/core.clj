  ...
  (loop [idx start-idx
    ...
    (if (>= attempts (count table))
      ...
      (let [slot (nth table idx)]
        (cond
          (or (nil? slot) (= slot :deleted))
          idx

          (and (vector? slot) (= (first slot) key))
          idx

          :else
          (recur (mod (inc idx) (count table)) (inc attempts)))))))
...
(deftype OADict [table]
  ...
  (cons [this o]
    (cond
      (instance? clojure.lang.IMapEntry o) (insert this (key o) (val o))
      (and (vector? o) (= 2 (count o)))    (insert this (o 0) (o 1))
      (map? o)                             (insert-all this o)
      (sequential? o)                      (insert-all this o)
      :else (throw (ex-info "Unsupported element for conj/cons on OADict" {:value o}))))
  (empty [_] (OADict. (vec (repeat TABLE_SIZE nil))))
  (equiv [this other]
    (cond
      (instance? OADict other) (equals-dict? this other)
