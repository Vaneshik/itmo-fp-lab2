...
Для работы со словарём реализованы функции-конструкторы:
...
Также реализована правая свёртка (стандартный `reduce` в Clojure работает только слева):
...
Полный код реализации: [core.clj](src/oa_dict/core.clj)
...
Полный код тестов приведён в файлах [unit_tests](test/oa_dict/unit_tests.clj) и [property_tests](test/oa_dict/property_tests.clj)
...
Для этого я переопределил обработку исключения для `dict/equals-dict?` таким образом чтобы при несоответствии вычислялись следующие списки:

- `only-in-1`: вхождения которые есть только в первом словаре

- `only-in-2`: вхождения которые есть только во втором словаре

- `mismatched`: словарь вида `k: [v1 v2]` отражающий что ключ `k` есть в обоих словарях но значения по нему различны

Для получения этих данных я написал функцию `dict-diff`

Все эти функции содержатся в файле [assertions.clj](test/oa_dict/assertions.clj)
...
Исходный код размещён в файле [pretty_reporter.clj](test/oa_dict/pretty_reporter.clj)
